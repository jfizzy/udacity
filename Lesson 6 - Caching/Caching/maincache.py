...
import json #library for jsons

from google.appengine.api import memcache # memcache lib
...
class JSONPage(Handler):
    def get(self):
        ...
        pageurl = url.split('.')[0] # slice notation
        req = None
        try:
            req = urllib2.urlopen(pageurl)
        except urllib2.URLError:
            self.redirect('/')
        if req:
            js_obj = json.loads(req.content)
            print js_obj
            self.response.out.write(js_obj)
...
global last_update_perma
last_update_perma = time.time()

class PermaPage(Handler):
    def get(self, product_id):
        post = memcache.get(product_id)
        global last_update_perma
        if post is None:
            logging.error("Post Query")
            post = Post.get_by_id(int(product_id)) # this is where we need to set the timer (helper method)
            memcache.set(product_id, post)
            last_update_perma = time.time()
        time_since = int(time.time()-last_update_perma)
        if self.format == 'html':
            self.render("post.html", title="Post %s" % product_id, post=post, lupdate=time_since)
        else:
...
global last_update_front
last_update_front = time.time()

def top_posts(update = False): # method checks if memcache has valid content
    key = 'top'                # or if we need to query the DB
...
class MainPage(Handler):
    def render_front(self, ptitle="", pbody="", created="", posts=""):
        posts = top_posts() # run the default memcache query
        global last_update_front
        time_since = int(time.time()-last_update_front)
        if self.format == 'html':
            self.render("front.html", title="Blog", subject=ptitle, content=pbody, created=created, posts=posts, lupdate=time_since)
        else:
