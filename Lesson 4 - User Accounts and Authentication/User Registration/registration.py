...
global last_user
last_user = ""
global last_pass #variables to internally store the last user's info
last_pass = ""
...
class SignupPage(Handler):
    def write_form(self, username="", user_error="", exist_error="", password="", pass_error="", verify="", veri_error="", email="", mail_error=""):
        self.render("signup.html", title="User Signup", username=username,
                    password=password,
                    verify=verify,
                    email=email,
    ...
    def post(self):
        ...
        if(errors == 0):
            ...
            if h:
                cookie_val = valid_hash(usr, pw, h)
                if cookie_val:
                    salt = h.split('|')[1]
                    self.write_form(usr,'',exist_err,'','','','',mail,'')
            
            h = make_pw_hash(usr, pw) # hash the user token
            global last_user
            global last_pass
            last_user = usr
            last_pass = pw
...
class LandingPage(Handler):
    def get(self):
        ...
        if valid_hash(last_user, last_pass, h):
            self.render("welcome.html", title="Welcome, %s!" % last_user)
        else:
